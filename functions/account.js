...
exports.updateAccount = async (event) => {
    ...
    try {
        ...
        if (data) {
            ...
            //Get account
            const account = await database.collection('accounts').findOne({ _id: ObjectId(updateBody._id) });
            
            //Update Account
            await database.collection('accounts').updateOne({ _id: ObjectId(updateBody._id) }, { $set: { amount: updateBody.amount } });

            //Update user balance
            await database.collection('users').updateOne({ "email": data.user.email }, { $set: { balance: (user.balance - account.amount  + updateBody.amount), currency: user.currency } });
    ...
    try {
        ...
        if (data) {
            ...
            if (result.deletedCount === 1) {
                ...
                //Update user balance
                await database.collection('users').updateOne({ "email": data.user.email }, { $set: { balance: (user.balance - account.amount), currency: user.currency } });
...
const handler = async function (event, context) {
    try {
        ...
        } else if (event.path == '/.netlify/functions/account/update' && event.httpMethod == 'PUT') {
            result = await exports.updateAccount(event);
        }
        return {
